...
# from recordings of subjects performing daily activities while carrying smartphone.
...
download.data = function() {
        ...
        if (!file.exists("clean_data/UCI HAR Dataset")) {
                # download the data
                fileURL <- "https://d396qusza40orc.cloudfront.net/getdata%2Fprojectfiles%2FUCI%20HAR%20Dataset.zip"
                zipfile="clean_data/UCI_HAR_data.zip"
...
merge.datasets = function() {
        ...
        training.x <- read.table("clean_data/UCI HAR Dataset/train/X_train.txt")
        message("reading y_train.txt")
        training.y <- read.table("clean_data/UCI HAR Dataset/train/y_train.txt")
        message("reading subject_train.txt")
        training.subject <- read.table("clean_data/UCI HAR Dataset/train/subject_train.txt")
        message("reading X_test.txt")
        test.x <- read.table("clean_data/UCI HAR Dataset/test/X_test.txt")
        message("reading y_test.txt")
        test.y <- read.table("clean_data/UCI HAR Dataset/test/y_test.txt")
        message("reading subject_test.txt")
        test.subject <- read.table("clean_data/UCI HAR Dataset/test/subject_test.txt")
        # Merge
